...
/// pixel is a (column, row) pair indicating the pixel in that image
/// upper_left and lower_right are points on the complex plane our image covers
fn pixel_to_point(
    bounds: (usize, usize),
    pixel: (usize, usize),
    upper_left: Complex<f64>,
    ...
    Complex {
        ...
        // we subtract here because the imaginary comonent increases as we go up
    }
}
...
// returns now many times it took to escape. none returned if
// it never escaped.
